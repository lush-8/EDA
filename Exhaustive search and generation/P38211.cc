...
bool BackTrack(int i, int n, vector<int>& queens, vector<bool>& columns, vector<bool>& diag, vector<bool>& antidiag) {
    ...
    for (int j = 0 ; j < n ; ++j) {
        if (not columns[j] and not diag[i + j] and not antidiag[n - i + j]) {
            ...
            if (BackTrack(i + 1, n, queens, columns, diag, antidiag)) return true ;
...
void Queens(int n) {
    ...
    if (not BackTrack(0, n, queens, columns, diag, antidiag)) {
